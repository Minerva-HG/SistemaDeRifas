...
function App() {
  ...
  const pagar = async () => {
    ...
    if (res.ok) setPagoConfirmado(true)
  }

  const comprarBoleto = async numero => {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/boletos/comprar`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ numero, comprador_id: compradorId })
    })
    if (res.ok) alert('✅ Boleto comprado')
    else alert('❌ Boleto no disponible')
  }

  if (compradorId === null) {
    return (
      <>
        <header style={{ background:'#0f172a', color:'white', padding:'14px', textAlign:'center', fontWeight:'bold' }}>
